package com.qianwang365.confkeeper.server.manager.impl;
 ...
 * @version 1.0
 */
public class APIConnector implements IAPIConnector<NameValuePair, JSONObject> {

	private static final Logger log = LoggerFactory
			.getLogger(APIConnector.class);
	
	private static final Integer TIMEOUT = 600;

	private final PropertyUtils pu = PropertyUtils.getInstance();
	...
	public APIConnector(boolean useProxy) {
		ThreadSafeClientConnManager mgr = new ThreadSafeClientConnManager();
		mgr.setMaxTotal(200);
		mgr.setDefaultMaxPerRoute(20);
		this.httpClient = new DefaultHttpClient(mgr);
		if (useProxy) {
			HttpHost proxy = new HttpHost(pu.getValue("ckmgr.server.proxy.ip",
					"127.0.0.1"), Integer.parseInt(pu.getValue(
					"ckmgr.server.proxy.port", "8888")));
	...
	public InputStream doPost4Stream(String targetUrl, String referUrl,
			List<NameValuePair> requestHeaders) throws ClientProtocolException,
			IOException {
		return this.doPost4Stream(targetUrl, referUrl, requestHeaders, null,
				null);
	}
	...
	public byte[] doPost4Bytes(String targetUrl, String referUrl,
			List<NameValuePair> requestHeaders) throws IOException {
		return this.doPost4Bytes(targetUrl, referUrl, requestHeaders, null,
				null);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.qianwang365.confkeeper.server.manager.IAPIConnector#invoke(com.
	 * qianwang365.confkeeper.server.manager.IAPIConnector.APIConnType,
	 * java.lang.String, java.lang.String, java.util.List, java.util.List,
	 * java.lang.String)
	 */
	@Override
	public JSONObject invoke(APIConnType method, String targetUrl,
			String referUrl, List<NameValuePair> requestHeaders,
			List<NameValuePair> requestEntity, String requestEntityCharset)
			throws ParseException, IOException {
		if (method == APIConnType.GET) {
			return JSONObject.parseObject(this.doGet4Html(targetUrl, referUrl,
					requestHeaders, requestEntity, requestEntityCharset));
		} else if (method == APIConnType.POST) {
			return JSONObject.parseObject(this.doPost4Html(targetUrl, referUrl,
					requestHeaders, requestEntity, requestEntityCharset));
		}
	...
	public static void main(String[] args) throws Exception {
		APIConnector client = new APIConnector(false);
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("entry", "wØeibo"));
		...
		String page = client.doPost4Html(
				"http://127.0.0.1:9012/myHttpService?a=1", null, null, params,
				"UTF-8");
		log.debug(page);
	}
}
