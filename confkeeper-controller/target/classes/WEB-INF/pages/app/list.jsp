			...
			<div id="content" class="span10">
				...
				<div>
					<ul class="breadcrumb">
						...
						<li><a href="javascript:void(0);">应用列表</a></li>
					</ul>
				</div>
				...
				<div class="row-fluid sortable">
					<div class="box span12">
						<div class="box-header well" data-original-title>
							<h2>
								<i class="icon-th-large"></i> 应用列表
							</h2>
							<div class="box-icon">
								<a href="${pageContext.request.contextPath}/app/add" class="btn btn-setting btn-round"><i class="icon-plus"></i></a>
								...
								<tbody>
									<c:forEach items="${datas}" var="obj">
										<tr>
											...
											<td>${obj.appNote}</td>
											<td>
												<c:forEach items="${obj.versions}" var="ep">
												<c:choose>
													<c:when test="${ep.state==1}">
														<div>
															<span class="label label-success">${ep.trunk}</span>
														</div>
													</c:when>
													<c:otherwise>
														<div>
															<span class="label label-important">${ep.trunk}</span>
														</div>
													</c:otherwise>
												</c:choose>
												</c:forEach>
											...
											<td class="center">
											<a class="btn btn-info" href="${pageContext.request.contextPath}/app/${obj.id}"> <i
													class="icon-edit icon-white"></i> 修改
											</a> <a class="btn btn-danger" href="${pageContext.request.contextPath}/app/remove/${obj.id}"> <i
